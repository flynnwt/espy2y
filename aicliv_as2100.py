...
# curl http://192.168.0.2/light/led_1
# curl http://192.168.0.2/switch/relay_1
#
# curl -d "" http://192.168.0.2/switch/relay_1/toggle
...
wifi = ('ssid', 'jennyjenny')
pw = ('password', '18008675309')
...
top.api(Properties([pw, ('reboot_timeout', '0s')]))
top.ota(Properties([('safe_mode', True), pw]))
top.web_server(Properties([('port', 80)]))
...
pin = Pin().number('GPIO4').inverted(True)
top.add('output', GPIO().id('status').pin(pin))
led1 = BinaryLight().name('LED 1').id('led1').output('status')
top.light(led1)

# --- Sensor ----------------------------------------------------------------------------------------------

pin = Pin().number('GPIO13').inverted(True)()  # WHY NEED ()???
button = GPIOBinarySensor().name('Button 1').id('button1').pin(pin)
...
relay1 = GPIOSwitch().name('Relay 1').id('relay1').pin('GPIO12').restore_mode('RESTORE_DEFAULT_OFF')
top.switch(relay1)
relay2 = GPIOSwitch().name('Relay 2').id('relay2').pin('GPIO14').restore_mode('RESTORE_DEFAULT_OFF')
top.switch(relay2)
relay3 = GPIOSwitch().name('Relay 3').id('relay3').pin('GPIO3').restore_mode('RESTORE_DEFAULT_OFF')
top.switch(relay3)
